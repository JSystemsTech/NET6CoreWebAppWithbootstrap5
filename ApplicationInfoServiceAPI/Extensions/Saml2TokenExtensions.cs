...
namespace ApplicationInfoServiceAPI.Extensions
{
    ...
    public class AuthTokenResponse : APIResponse
    {
        ...
        public static AuthTokenResponse Error(string errorMessage)
    ...
    public static class Saml2TokenExtensions
    {
        ...
        {
            Saml2SubjectConfirmationData confirmationData = new Saml2SubjectConfirmationData() { Address = ApplicationConfiguration.ApplicationSettings.ConfirmationMethod };
            Saml2SubjectConfirmation subjectConfirmations = new Saml2SubjectConfirmation(new Uri(ApplicationConfiguration.ApplicationSettings.ConfirmationMethod), confirmationData);
            Saml2AudienceRestriction[] audienceRestriction = new Saml2AudienceRestriction[1] { new Saml2AudienceRestriction(ApplicationConfiguration.ApplicationSettings.AudienceUri is Uri uri ? uri.ToString() : "") };
            Saml2Assertion assertion = new Saml2Assertion(new Saml2NameIdentifier(ApplicationConfiguration.ApplicationSettings.Issuer))
            {
                ...
                InclusiveNamespacesPrefixList = ApplicationConfiguration.ApplicationSettings.Namespace,
                Subject = new Saml2Subject(subjectConfirmations)
                {
                    NameId = new Saml2NameIdentifier(ApplicationConfiguration.ApplicationSettings.ConfirmationMethod)
                }
            };
        ...
        private static bool IsValidToken(Saml2SecurityToken saml2Token)
        {
            ...
            return isValidIssueDate && isValidExpirationDate && ApplicationConfiguration.ApplicationSettings.Issuer == saml2Token.Assertion.Issuer.Value;
        }
        ...
        {
            int validForFinal = ApplicationConfiguration.ApplicationSettings.ValidFor;
            DateTime utcNow = DateTime.UtcNow;
